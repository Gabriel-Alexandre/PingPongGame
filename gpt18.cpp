...
void draw() {
    ...
    if (varStart || gameOver) {
        ...
        glColor3f(255.0f, 165.0f, 0.0f); // Cor verde para o botão "Start Game"
        glBegin(GL_QUADS);
    ...
    if (!gameOver) {
        ...
        glVertex2f(-0.93f, paddle1Y + paddleHeight / 2);// laura
        glVertex2f(-0.95f, paddle1Y + paddleHeight / 2);
    ...
    if (!gamePaused && !gameOver && !varStart) {
        ...
        if ((ballX - ballRadius) < (-0.93f) && (ballY >= paddle1Y - paddleHeight / 2) && (ballY <= paddle1Y + paddleHeight / 2)) {
            ...
            if(isHard) {
                ...
                std::vector<float> valueList = {-0.5f, -0.9f, -0.95f, -1.2f, -1.5f, -1.6f, -1.77f, -1.8f, -1.85f,  0.5f, 0.85f, 0.9f, 0.95f, 0.99f};
                angleBall = randomDirection(valueList);
            }
...
void mouseClick(int button, int state, int x, int y) {
    ...
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
        ...
        if(startButtonVisible && ((x >= 320 && x <= 481 && y >= 270 && y <= 333) || (x >= 745 && x <= 1115 && y >= 457 && y <= 561))) {
            startButtonVisible = false;
            isHard = true;
            alSourceStop(gameStartSound);
            alSourcePlay(gamingSound);
